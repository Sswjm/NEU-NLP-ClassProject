...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        #layer 2
        self.w_ii_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_ii_2 = nn.Parameter(torch.ones([n_hidden]))

        self.w_hi_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_hi_2 = nn.Parameter(torch.ones([n_hidden]))

        self.w_if_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_if_2 = nn.Parameter(torch.ones([n_hidden]))

        self.w_hf_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_hf_2 = nn.Parameter(torch.ones([n_hidden]))

        self.w_ig_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_ig_2 = nn.Parameter(torch.ones([n_hidden]))

        self.w_hg_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_hg_2 = nn.Parameter(torch.ones([n_hidden]))

        self.w_io_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_io_2 = nn.Parameter(torch.ones([n_hidden]))

        self.w_ho_2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.b_ho_2 = nn.Parameter(torch.ones([n_hidden]))
    ...
    def forward(self, hidden, X):
        ...
        for x_t in outputs:
            i_t = self.sigmoid(self.w_ii_2(x_t) + self.b_ii_2 + self.w_hi_2(h_t_1) + self.b_hi_2)
            f_t = self.sigmoid(self.w_if_2(x_t) + self.b_if_2 + self.w_hf_2(h_t_1) + self.b_hf_2)
            g_t = self.tanh(self.w_ig_2(x_t) + self.b_ig_2 + self.w_hg_2(h_t_1) + self.b_hg_2)
            o_t = self.sigmoid(self.w_io_2(x_t) + self.b_io_2 + self.w_ho_2(h_t_1) + self.b_ho_2)
            c_t = f_t * c_t_1 + i_t * g_t
            h_t = o_t * self.tanh(c_t)
